...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Insert review
    if (!empty($review)) {
        $stmt = $conn->prepare("INSERT INTO admins_reviews (admin_id, name, review, images) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("isss", $admin_id, $name, $review, $imagesJSON);
        $stmt->execute();
        $stmt->close();
                      ...
                      <div class="review-content">
                        ...
                        <div class="card mt-3 p-3" style="background-color: #f1f5f9; border-left: 4px solid #4361ee;">
                          ...
                          <span style="font-size: 13px; color: <?= (!empty($row['admin_reply']) && $row['admin_reply_read']) ? '#28a745' : '#ff0000'; ?>;">
                            <?= (!empty($row['admin_reply']) && $row['admin_reply_read']) ? 'Read' : 'Not read yet'; ?>
                          </span>
                        </div>
                      </div>
                    </div>
