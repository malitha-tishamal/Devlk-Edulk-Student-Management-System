...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Insert review
    if (!empty($review)) {
        $stmt = $conn->prepare("INSERT INTO student_reviews (student_id, name, regno, batch_year, review, images) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("isssss", $student_id, $name, $regno, $batch_year, $review, $imagesJSON);
        $stmt->execute();
        $stmt->close();
    ...
    <div class="row">
      <div class="col-12">
        <div class="card">
          ...
          <div class="card-body">
            <?php
            $result = $conn->query("SELECT name, regno, batch_year, review, images, created_at FROM student_reviews ORDER BY created_at DESC LIMIT 10");
