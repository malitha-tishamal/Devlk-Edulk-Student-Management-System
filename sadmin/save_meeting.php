...
if ($title && $date && $start_time && $zoom_link && $subject) {
    $stmt = $conn->prepare("INSERT INTO meetings (title, date, start_time, zoom_link, created_by, role, status, subject) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssss", $title, $date, $start_time, $zoom_link, $user_name, $user_role, $status, $subject);
