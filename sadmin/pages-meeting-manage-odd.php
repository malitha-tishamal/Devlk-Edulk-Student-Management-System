...
<script>
  ...
  tbody.innerHTML += `
    <tr data-id="${row.id}">
      ...
      <td>
        <button class="btn btn-success btn-sm action-btn ${row.status === 'active' ? 'disabled-btn' : ''}" data-action="activate" ${row.status === 'active' ? 'disabled' : ''}>Activate</button>
        <button class="btn btn-warning btn-sm action-btn ${row.status === 'disabled' ? 'disabled-btn' : ''}" data-action="disable" ${row.status === 'disabled' ? 'disabled' : ''}>Disable</button>
...
document.body.addEventListener('click', e => {
  if (e.target.classList.contains('toggle-resource-btn')) {
    ...
    const currentStatus = btn.getAttribute('data-status'); // "active" or "disabled"
    const newStatus = currentStatus === "disabled" ? "active" : "disabled";

    if (!confirm(`Are you sure you want to ${newStatus === "disabled" ? "disable" : "activate"} this resource?`)) return;
    ...
    .then(resp => {
      if (resp.success) {
        alert(`Resource has been ${newStatus === "disabled" ? "disabled" : "activated"} successfully.`);
        // Reload resources list
        // Get meetingId from the resource list container id: resource-list-{meetingId}
        ...
        alert("Failed to update resource status: " + (resp.message || 'Unknown error'));
      }
    })
